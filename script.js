...
function draw() {
    ...
    if (!startGame) {
        ...
        function drawButton(strokeColor, fillColor) {
            ...
            bauttonStart.rect(xButton, yButton, widthButton, heightButton);
            drawPath(bauttonStart, strokeColor, fillColor, 3);
            let buttonText = 'START';
            printText(buttonText,xButton+widthButton/2-innerWidth*0.02,yButton+heightButton/2+innerHeight*0.01,25,"Black");
        }
        drawButton('Crimson','HotPink');
        canvas.addEventListener('mousemove',(Event)=>{
            if(ctx.isPointInPath(bauttonStart,Event.clientX,Event.clientY)){
                drawButton('Crimson','HotPink');
            }else{
                drawButton('Teal', 'Maroon')
            }
        })
